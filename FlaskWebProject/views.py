...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user is None or not user.check_password(form.password.data):
            app.logger.warning("Logging Failed with user: {}".format(form.username.data))
            flash('Invalid username or password')
            return redirect(url_for('login'))
        app.logger.info("Log in successful with user {}".format(form.username.data))
...
def logout():
    ...
    if session.get("user"): # Used MS Login
        ...
        app.logger.info("Log out user: {}".format(session.get("user")))
        session.clear()
